...
pub struct DockArea {
    ...
    items: DockItem,
    /// The most recently focused tab panel in this dock area.
    last_focused_tab_panel: Option<WeakEntity<TabPanel>>,
...
impl DockItem {
    ...
    /// Collect all TabPanels in this DockItem tree.
    pub fn all_tab_panels(&self, cx: &App) -> Vec<Entity<TabPanel>> {
        let mut result = Vec::new();
        self.collect_tab_panels(&mut result, cx);
        result
    }
    ...
    fn collect_tab_panels(&self, out: &mut Vec<Entity<TabPanel>>, cx: &App) {
        match self {
            ...
            DockItem::Tiles { items: _, .. } => {
                // Tiles contain panels directly, not nested DockItems
            }
            DockItem::Panel { .. } => {}
        }
    }
...
impl DockArea {
    ...
    /// Returns true if a panel is currently zoomed.
    ...
    /// Remember the most recently focused tab panel for action routing when
    /// focus temporarily moves to overlays like palettes or sidebars.
    pub fn remember_tab_panel(&mut self, tab_panel: WeakEntity<TabPanel>) {
        self.last_focused_tab_panel = Some(tab_panel);
    }

    fn remembered_tab_panel(&self, cx: &App) -> Option<Entity<TabPanel>> {
        let remembered = self.last_focused_tab_panel.as_ref()?.upgrade()?;
        let remembered_id = remembered.entity_id();

        self.items
            .all_tab_panels(cx)
            .into_iter()
            .find(|tab_panel| tab_panel.entity_id() == remembered_id)
    }

    /// Return the best tab panel target for commands.
    ///
    /// This prefers the currently focused tab panel, then the most recently
    /// focused tab panel in this dock, and finally the first available tab panel.
    pub fn target_tab_panel(&self, window: Option<&Window>, cx: &App) -> Option<Entity<TabPanel>> {
        if let Some(window) = window {
            if let Some(focused) = self.focused_tab_panel(window, cx) {
                return Some(focused);
            }
        }

        self.remembered_tab_panel(cx)
            .or_else(|| self.items.all_tab_panels(cx).into_iter().next())
    }
    ...
    pub fn set_center(&mut self, item: DockItem, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.items = item;
        self.last_focused_tab_panel = None;
    ...
    /// Find the TabPanel that currently contains the focused element.
    pub fn focused_tab_panel(&self, window: &Window, cx: &App) -> Option<Entity<TabPanel>> {
        use gpui::Focusable;
        self.items.all_tab_panels(cx).into_iter().find(|tp| {
            let handle = tp.read(cx).focus_handle(cx);
