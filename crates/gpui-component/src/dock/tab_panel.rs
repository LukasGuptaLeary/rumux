...
use gpui::{
    ...
    MouseDownEvent, ParentElement, Pixels, Render, ScrollHandle, SharedString,
...
impl TabPanel {
    ...
    /// Return the number of panels in this TabPanel.
    ...
    pub fn set_active_index(&mut self, ix: usize, window: &mut Window, cx: &mut Context<Self>) {
    ...
    fn render_title_bar(
        ...
        let title_bar_bg = if state.focused {
            cx.theme().tab_bar.blend(cx.theme().primary.opacity(0.06))
        } else {
            cx.theme().tab_bar
        };
            ...
            .children(self.panels.iter().enumerate().filter_map(|(ix, panel)| {
                ...
                Some(
                    Tab::default()
                        ...
                        .when(
                            panel.closable(cx)
                                && (self.panels.len() > 1 || (active && self.closable(cx))),
                            |this| {
                                this.suffix(
                                    div()
                            ...
                            cx.listener({
                                ...
                                move |_, event: &MouseDownEvent, window, cx| {
                                    if event.click_count == 2 {
                                        panel.on_tab_double_click(window, cx);
                                    }
                                }
                            }),
                        )
